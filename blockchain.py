...
class Blockchain(object):
    ...
    def new_transaction(self, sender, recipient, amount): # Adds a new transaction to the list of transactions
    ...
    def valid_chain(self, chain): # Determine if a given blockchain is valid
        last_block = chain[0]
        current_index = 1
...
def new_transaction():
    ...
    required = ['sender', 'recipient', 'amount'] # Check that the required fields are in the POST'ed data
    if not all(k in values for k in required):
        return 'Missing values', 400

    index = blockchain.new_transaction(values['sender'], values['recipient'], values['amount']) # Create a new Transaction
